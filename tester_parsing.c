...
// void handle_redirection(t_cmd *list, T_list *token)
// {
// 	if (!list->file)
// 	{
...
void handle_redirection(t_cmd *list, T_list *token)
{
	t_red_list *new = malloc(sizeof(t_red_list));
	if (!new)
		return ;
...
void splitit(T_list *token, t_cmd **final)
{
    ...
    t_cmd *cmd_head = NULL;
    t_cmd *current = NULL;
	// t_cmd *new_cmd = create_new_cmd(array_size);
	// if (!new_cmd)
	while (tmp)
	{
	...
	while (tmp)
	{
		...
		t_cmd *new_cmd = create_new_cmd(word_count);
		if (!new_cmd)
			return;
