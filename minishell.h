...
/*   Updated: 2025/08/15 17:00:28 by salhali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void						init_all(t_pars *p, t_token *t, t_cmd *cmd,
								t_wlist *w);
char						*get_env_value_ll(t_env *env, const char *key);
char						*find_last_valid_dollar(char *str);
int							get_varname_length(char *start);
char						*get_variable_value(char *var_start, int var_len,
								t_shell *shell);
char						*build_result_string(char *input, char *dollar_pos,
								char *env_value, int var_len);
char						*find_first_valid_dollar(char *str);
char						*expand_variables_fixed(char *input,
								t_shell *shell);
...
int							check3(char *sa);
char						*call_expand(t_pars *pars, t_shell *shell,
								char *part);
void						fill_array_plus(t_pars *pars);
void						count_pipes(t_pars *pars);
char						*call_expand_in_q(t_pars *pars, char *segment,
								t_shell *shell);
...
void						update_env_variable(t_shell *shell,
								const char *name, const char *value);
void						delete_env_variable(t_shell *shell,
								const char *name);
...
int							handle_cd_change(char *path, char *current_dir,
								t_shell *shell);
...
void						handle_heredoc(t_red_list *tmp, t_red_list *head,
								t_shell *shell, t_cmd *clist);
void						ft_waitpid(pid_t *pids, int i, t_shell *shell);
void						execute(t_cmd *clist, t_wlist *wlist,
								t_shell *shell);
int							count_dollars(char *sa);
t_env						*find_env_variable(t_shell *shell, const char *key);
int							update_export_variable(t_shell *shell,
								const char *key, const char *value);
int							create_export_variable(t_shell *shell,
								const char *key, const char *value);
int							handle_export_with_value(t_shell *shell, char *arg);
int							handle_export_without_value(t_shell *shell,
								const char *arg);
void						signe(void);
...
int							read_heredoc_line(char **line, size_t *len,
								char *delimiter);
void						cleanup_heredoc(t_red_list *head, char *line,
								int fd);
void						heredoc_input(char *delimiter, t_red_list *head,
								t_shell *shell, t_cmd *clist);
void						setup_pipeline(t_cmd *clist, int *pipe_fd);
void						handle_parent_process(int *in_fd, int *pipe_fd,
								t_cmd *clist);
void						execute_external_command(t_cmd *clist, char **envp);
void						handle_child_process(t_cmd *clist, t_shell *shell,
								int in_fd, int *pipe_fd);
void						execute_cmds(t_cmd *clist, t_shell *shell);
char						*build_env_string(const char *key,
								const char *value);
int							count_env_variables(t_shell *shell);
t_env						*create_env_node(const char *env_str);
void						append_env_node(t_env **head, t_env **last,
								t_env *node);
void						setup_child_input_output(int in_fd, t_cmd *clist,
								int *pipe_fd);
void						handle_builtin_or_external(t_cmd *clist,
								t_shell *shell, char **envp);
void						cleanup_shell(t_shell *shell);
void						shell_loop(t_shell *shell);
void						cleanup_iteration(t_wlist **wlist, t_cmd **clist,
								char *input);
void						process_input(char *input, t_wlist **wlist,
